...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers(HttpMethod.GET,"/Alumno/*").authenticated() //cualquiera que este autenticado
		.and() 
		.addFilter(new JwtFilter(authenticationManager(),clave));
		//httpBasic() manera en la que se solicita los credenciales. 
		
		//ACCESO LIBRE. SIN USUARIO NI CONTRASEÑA.
		//Alumno(s) y Curso(s) cualquiera puede acceder. METODO LIBRE GET ALUMNO(S) CURSO(S) No se ha dicho nada de este.
